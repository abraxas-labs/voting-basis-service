...
public class SecondaryMajorityElectionListTest : BaseGrpcTest<MajorityElectionService.MajorityElectionServiceClient>
{
    private const string IdNotFound = "eae2cfaf-c787-48b9-a108-c975b0addddd";

    public SecondaryMajorityElectionListTest(TestApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    {
        var response = await AdminClient.ListSecondaryMajorityElectionsAsync(new ListSecondaryMajorityElectionsRequest
        {
    ...
    {
        var response = await ElectionAdminClient.ListSecondaryMajorityElectionsAsync(new ListSecondaryMajorityElectionsRequest
        {
            MajorityElectionId = MajorityElectionMockedData.IdGossauMajorityElectionInContestStGallen,
        });
        response.MatchSnapshot();
    ...
    {
        await AssertStatus(
            async () => await AdminClient.ListSecondaryMajorityElectionsAsync(new ListSecondaryMajorityElectionsRequest
            {
    ...
    {
        await AssertStatus(
            async () => await ElectionAdminClient.ListSecondaryMajorityElectionsAsync(new ListSecondaryMajorityElectionsRequest
            {
                MajorityElectionId = MajorityElectionMockedData.IdBundMajorityElectionInContestStGallen,
            }),
            StatusCode.InvalidArgument);
    ...
    protected override async Task AuthorizationTestCall(GrpcChannel channel)
    {
        await new MajorityElectionService.MajorityElectionServiceClient(channel)
            .ListSecondaryMajorityElectionsAsync(new ListSecondaryMajorityElectionsRequest
            {
