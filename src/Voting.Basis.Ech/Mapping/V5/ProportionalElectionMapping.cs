...
internal static class ProportionalElectionMapping
{
    ...
    internal static EventInitialDeliveryElectionGroup ToEchElectionGroup(this ProportionalElection proportionalElection, bool eVoting)
    {
        var electionInfo = proportionalElection.ToEchElectionInformation(eVoting);
        var electionGroupBallot = new EventInitialDeliveryElectionGroup
        {
            DomainOfInfluence = proportionalElection.DomainOfInfluence!.ToEchDomainOfInfluence(),
            ElectionInformation = new List<EventInitialDeliveryElectionGroupElectionInformation> { electionInfo },
        };
    ...
    private static EventInitialDeliveryElectionGroupElectionInformation ToEchElectionInformation(this ProportionalElection proportionalElection, bool eVoting)
    {
        ...
        var lists = proportionalElection.ProportionalElectionLists
            ...
            .Select(l => l.ToEchListType(canton, eVoting))
            .Append(CreateEmptyListType(proportionalElection, eVoting))
            .ToList();
        ...
        var candidates = proportionalElection.ProportionalElectionLists
            ...
            .ThenBy(c => c.Number)
            .Select(c => c.ToEchProportionalCandidateType(canton, eVoting))
            .ToList();
        ...
        var listUnions = proportionalElection.ProportionalElectionListUnions
            ...
            .Select(u => ToEchListUnionType(u, eVoting))
            .ToList();
    ...
    private static ListType ToEchListType(this ProportionalElectionList list, DomainOfInfluenceCanton canton, bool eVoting)
    {
        var descriptionInfos = new ListDescriptionInformationType();
        var descriptionTranslations = list.Description.FilterEchExportLanguages(eVoting);
        var shortDescriptionTranslations = list.ShortDescription.FilterEchExportLanguages(eVoting);

        foreach (var (lang, desc) in descriptionTranslations)
        {
            shortDescriptionTranslations.TryGetValue(lang, out var shortDescription);
        ...
        {
            candidatePositions.Add(candidate.ToEchCandidatePosition(false, canton, eVoting));
            if (candidate.Accumulated)
            {
                candidatePositions.Add(candidate.ToEchCandidatePosition(true, canton, eVoting));
            }
        }
    ...
    private static ListType CreateEmptyListType(ProportionalElection proportionalElection, bool eVoting)
    {
        var descriptionInfos = LanguageMapping.GetEchExportLanguages(eVoting)
    ...
    private static ListUnionType ToEchListUnionType(this ProportionalElectionListUnion listUnion, bool eVoting)
    {
    ...
    private static CandidatePositionInformationType ToEchCandidatePosition(this ProportionalElectionCandidate candidate, bool accumulatedPosition, DomainOfInfluenceCanton canton, bool eVoting)
    {
        var text = candidate.ToEchCandidateText(canton, eVoting, PoliticalBusinessType.ProportionalElection, candidate.Party?.Name);
        ...
        return new CandidatePositionInformationType
        {
            ...
            CandidateReferenceOnPosition = CandidateNumberUtils.GenerateCandidateReference(candidate.ProportionalElectionList.OrderNumber, candidate.Number),
